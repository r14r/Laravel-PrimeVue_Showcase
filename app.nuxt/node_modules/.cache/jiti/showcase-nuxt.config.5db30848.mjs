"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var _fs = _interopRequireDefault(await jitiImport("fs"));
...
const alias = {
  primevue: _path.default.resolve(__dirname, '../../packages/primevue/src'),
  '@primevue/core': _path.default.resolve(__dirname, '../../packages/core/src'),
  '@primevue/icons': _path.default.resolve(__dirname, '../../packages/icons/src')
};
...
const markdownRedirects = (() => {
  const rules = {};
  const llmsDir = _path.default.resolve(__dirname, 'server/assets/llms');
  ...
  try {
    ...
    for (const file of _fs.default.readdirSync(_path.default.join(llmsDir, 'pages'))) {
      rules[`/${file}`] = { redirect: { to: `/llms/pages/${file}`, statusCode: 301 } };
    }

    // Add component redirects
    for (const file of _fs.default.readdirSync(_path.default.join(llmsDir, 'components'))) {
      if (!rules[`/${file}`]) {
